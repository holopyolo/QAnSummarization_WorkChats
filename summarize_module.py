...
logger = logger_logic.get_logger(__name__)
...
with open('config_models.json', 'r', encoding='utf-8') as fl:
    ...
    logging_level = config_js['logging_level']
...
model = AutoModelForCausalLM.from_pretrained(
    ...
    device_map="auto",
    # (vllm requires >16 gb, so we use flash_attention2)
...
def prepare(model, tokenizer, query, prompt) -> dict:
    ...
    logger.info('query and prompt are prepared')
    return data
...
def get_ans(model, tokenizer, data, custom_config=None) -> str:
    ...
    '''
    if custom_config:
        logger.info('Custom config is used')
        output_ids = model.generate(**data, generation_config=custom_config)[0]
    else:
        logger.info('Default config is used, config is not boot up')
        output_ids = model.generate(
            **data, generation_config=generation_config)[0]
